 ...
 * @Date: 2023-02-27 17:47:12
 * @LastEditors: SnotlingLiu<snotlingliu@gmail.com/>
 * @LastEditTime: 2023-02-27 18:30:10
 * @Description: 
 */

import buildModuleUrl from "../../Source/Core/buildModuleUrl";
import Cartesian3 from "../../Source/Core/Cartesian3";
import Clock from "../../Source/Core/Clock";
...
import Ellipsoid from "../../Source/Core/Ellipsoid";
import FeatureDetection from "../../Source/Core/FeatureDetection";
import formatError from "../../Source/Core/formatError";
import ScreenSpaceEventHandler from "../../Source/Core/ScreenSpaceEventHandler";
import Globe from "../../Source/Scene/Globe";
import Moon from "../../Source/Scene/Moon";
import Scene from "../../Source/Scene/Scene";
import SceneMode from "../../Source/Scene/SceneMode";
import ShadowMode from "../../Source/Scene/ShadowMode";
import SkyAtmosphere from "../../Source/Scene/SkyAtmosphere";
import SkyBox from "../../Source/Scene/SkyBox";
import Sun from "../../Source/Scene/Sun";
...
function startRenderLoop(space) {
  space._renderLoopRunning = true;
  ...
  function render(frameTime) {
    ...
    if (space._useDefaultRenderLoop) {
      try {
        const targetFrameRate = space._targetFrameRate;
        ...
        space._useDefaultRenderLoop = false;
        space._renderLoopRunning = false;
        if (space._showRenderLoopErrors) {
          const title =
            "An error occurred while rendering.  Rendering has stopped.";
          space.showErrorPanel(title, undefined, error);
        }
      }
    } else {
      space._renderLoopRunning = false;
...
function configurePixelRatio(space) {
  let pixelRatio = space._useBrowserRecommendedResolution
    ? 1.0
    : window.devicePixelRatio;
  pixelRatio *= space._resolutionScale;
  if (defined(space._scene)) {
    space._scene.pixelRatio = pixelRatio;
  }

  return pixelRatio;
}

function configureCanvasSize(space) {
  const canvas = space._canvas;
  let width = canvas.clientWidth;
  let height = canvas.clientHeight;
  const pixelRatio = configurePixelRatio(space);

  space._canvasClientWidth = width;
  space._canvasClientHeight = height;
  ...
  space._canRender = width !== 0 && height !== 0;
  space._lastDevicePixelRatio = window.devicePixelRatio;
}

function configureCameraFrustum(space) {
  const canvas = space._canvas;
  const width = canvas.width;
  const height = canvas.height;
  if (width !== 0 && height !== 0) {
    const frustum = space._scene.camera.frustum;
...
/**
 * A space containing a Cesium scene.
 *
 * @alias Space
 * @constructor
 *
 ...
 *
 * @example
 * // For each example, include a link to Space.css stylesheet in HTML head,
 * // and in the body, include: <div id="cesiumContainer"></div>
 *
 * //Widget with no terrain and default Bing Maps imagery provider.
 * const widget = new Cesium.Space('cesiumContainer');
 *
 * //Widget with ion imagery and Cesium World Terrain.
 * const widget2 = new Cesium.Space('cesiumContainer', {
 ...
 * });
 */
function Space(container, options) {
  ...
  //Configure the space DOM elements
  const element = document.createElement("div");
  element.className = "spaceMain";
  ...
  innerCreditContainer.className = "spaceMain-credits";
  ...
  try {
    ...
    if (showRenderLoopErrors) {
      const title = "Error constructing Space.";
      const message =
...
Object.defineProperties(Space.prototype, {
  ...
  /**
   * Gets or sets the target frame rate of the space when <code>useDefaultRenderLoop</code>
  ...
  /**
   * Gets or sets whether or not this space should control the render loop.
   * If true the space will use requestAnimationFrame to
   * perform rendering and resizing of the space, as well as drive the
   ...
   * For example, if the space is laid out at a size of 640x480, setting this value to 0.5
   ...
   * will be in device pixels. {@link Space#resolutionScale} will still take effect whether
 ...
 */
Space.prototype.showErrorPanel = function (title, message, error) {
  const element = this._element;
  const overlay = document.createElement("div");
  overlay.className = "spaceMain-errorPanel";

  const content = document.createElement("div");
  content.className = "spaceMain-errorPanel-content";
  overlay.appendChild(content);

  const errorHeader = document.createElement("div");
  errorHeader.className = "spaceMain-errorPanel-header";
  errorHeader.appendChild(document.createTextNode(title));
  content.appendChild(errorHeader);

  const errorPanelScroller = document.createElement("div");
  errorPanelScroller.className = "spaceMain-errorPanel-scroll";
  ...
  if (hasMessage || hasError) {
    ...
    errorMessage.className = "spaceMain-errorPanel-message";
    ...
    if (hasError) {
      ...
      errorMessageDetails.className =
        "spaceMain-errorPanel-message-details collapsed";

      const moreDetails = document.createElement("span");
      moreDetails.className = "spaceMain-errorPanel-more-details";
      ...
      errorMessageDetails.onclick = function (e) {
        ...
        errorMessageDetails.className =
          "spaceMain-errorPanel-message-details";
        content.className = "spaceMain-errorPanel-content expanded";
  ...
  buttonPanel.className = "spaceMain-errorPanel-buttonPanel";
 ...
 */
Space.prototype.isDestroyed = function () {
  return false;
};

/**
 * Destroys the space.  Should be called if permanently
 * removing the space from layout.
 */
